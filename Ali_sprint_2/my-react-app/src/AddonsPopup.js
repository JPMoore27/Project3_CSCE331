...
const AddonsPopup = ({ addonsCart, onClose, onRemoveAddon, selectedAddons, onAddToCart }) => {
  ...
  return (
    <div style={popupStyles}>
      <div style={boxStyles}>
        ...
        <ul>
          {addonsData.map((addon, index) => (
            <li key={index} style={listItemStyles}>
              {addon.itemName} - ${addon.price.toFixed(2)} ({addon.dairy})
              <span>
        ...
        <button className="addons-add-to-cart" onClick={onClose}>
          Add to Cart
        </button>
      </div>
    </div>
